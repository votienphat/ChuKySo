...
namespace Common
{
    ...
    {
        protected string GetValueByKey<T>(object key)
        {
        ...
        //public List<T> GetAllByKey<T>(params object[] keys)
        //{
        //    FieldInfo[] p = GetType().GetFields(BindingFlags.Public | BindingFlags.Static);
        //    return p.Select(f => (ConstantsKeyValue)f.GetValue(null)).Where(t => t != null && keys.Contains(t.Key)).ToList();
        //}
    }
